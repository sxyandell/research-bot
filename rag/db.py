...
def build_create_table_sql(
    ...
    sql = (
        ...
        f"    base.* EXCLUDE (\"Which_mice\") REPLACE (\n"
        f"      CASE\n"
        f"        WHEN lower(phenotype_class) IN ('liver_gene','liver_genes') AND (\n"
        f"          gene_symbol IS NULL OR length(trim(gene_symbol)) = 0 OR lower(trim(gene_symbol)) = 'na'\n"
        f"        ) THEN gene_id\n"
        f"        ELSE gene_symbol\n"
        f"      END AS \"gene_symbol\",\n"
        f"      CASE\n"
        f"        WHEN regexp_matches(lower(phenotype_class), '^plasma_.*_metabolite$') THEN 'plasma_metabolite'\n"
        f"        ELSE phenotype_class\n"
        f"      END AS \"phenotype_class\"\n"
        f"    )\n"
        f"  FROM base\n"
        f")\n"
        f"SELECT\n"
